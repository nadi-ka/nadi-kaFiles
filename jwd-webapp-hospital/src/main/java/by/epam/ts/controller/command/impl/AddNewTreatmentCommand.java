...
public final class AddNewTreatmentCommand implements Command {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String staffId = (String) session.getAttribute(SessionAtribute.USER_ID);
		...
		//checking of date_finish, as it's allowed not to be fulfilled. By default, date_finish is equals date_begin;
		if (dateFinish == null) {
		...
		try {
			...
			response.sendRedirect(request.getContextPath() + RequestAtribute.CONTROLLER_FONT + RequestAtribute.COMMAND
					...
					+ RequestAtribute.MESSAGE + "=" + RequestMessage.TREATMENT_ADDED_SUCCESSFULY + "&"
			...
			log.log(Level.ERROR,
					"Error when calling userService.addNewTreatment() from  AddNewTreatmentCommand.",
					e);
