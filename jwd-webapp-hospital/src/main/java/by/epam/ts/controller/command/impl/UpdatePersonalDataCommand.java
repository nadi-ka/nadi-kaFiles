...
public final class UpdatePersonalDataCommand implements Command {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		log.info(surname + " " + name + " " + newEmail + " " + "old:" + oldEmail);
		...
		try {
			...
			log.log(Level.WARN,
					"Error when calling execute() from UpdatePersonalDataCommand. Invalid parameters:",
					e);
			response.sendRedirect(request.getContextPath() + RequestAtribute.CONTROLLER_FONT + RequestAtribute.COMMAND + "="
					+ CommandEnum.GET_UPDATE_PERSONAL_DATA_PAGE.toString().toLowerCase() + "&" + RequestAtribute.MESSAGE + "="
					+ RequestMessage.ERROR_DATA);
		}
		catch (ServiceException e) {
			log.log(Level.ERROR, "Error when calling execute() from UpdatePersonalDataCommand. The staff's personal data wasn't updated.", e);
			response.sendRedirect(request.getContextPath() + RequestAtribute.CONTROLLER_FONT + RequestAtribute.COMMAND + "="
