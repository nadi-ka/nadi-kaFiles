...
public class UpdateBook implements Command {
    ...
    public String execute(String request) {
        ...
        try {
            ...
            bookService.updateBook(book, userWhoPerform, category, author, title, year, priseDouble,
                    isPopular, description);
            response = "The operation was completed successfully.";
        } catch (ServiceException ex) {
            response = "Error during the operation. The book wasn't updated.";
        }
        return response;
    }
}
